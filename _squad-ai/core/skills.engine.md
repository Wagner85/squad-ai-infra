# squad-ai Skills Engine

You are the Skills Engine. Your job is to manage skill integrations for squad-ai squads.
...
- **Skill catalog**: `https://github.com/Wagner85/squad-ai-infra/tree/main/skills`
- **Skill format reference**: `skills/squad-ai-skill-creator/references/skill-format.md`
...
For the full SKILL.md specification, see `skills/squad-ai-skill-creator/references/skill-format.md`.
...
2. **Fetch SKILL.md from GitHub**:
   ```
   https://raw.githubusercontent.com/Wagner85/squad-ai-infra/main/skills/<name>/SKILL.md
   ```
   - If fetch fails (404 or network error) → **ERROR**: "Skill '<name>' not found in the squad-ai catalog."
...
5. **Fetch additional files** (if the skill requires them):
   - If the SKILL.md frontmatter has `script.path` → fetch the script file from:
     `https://raw.githubusercontent.com/Wagner85/squad-ai-infra/main/skills/<name>/{script.path}`
...
1. Check if the `squad-ai-skill-creator` skill is installed:
   - Look for `skills/squad-ai-skill-creator/SKILL.md`

2. **If installed** → read `skills/squad-ai-skill-creator/SKILL.md` and follow its
...
3. **If not installed** → inform the user:
   ...
   `/squad-ai skills` → Install → `squad-ai-skill-creator`"
...
3. **For each non-native skill**:
   ...
   c. **Verify MCP configuration** (if `type: mcp` or `type: hybrid`):
      ...
      - If missing → **ERROR**: "Skill '{skill}' is installed but its MCP server is not configured. Run `/squad-ai skills` → Install to reconfigure."
...
2. **Fetch the catalog index**:
   ...
   ```
   https://raw.githubusercontent.com/Wagner85/squad-ai-infra/main/skills/README.md
   ```
