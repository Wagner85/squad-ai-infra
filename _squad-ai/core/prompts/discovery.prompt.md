...
- `_squad-ai/_memory/company.md` — company name, tone, brand, products
- `_squad-ai/_memory/preferences.md` — user's preferred language, tools, defaults
...
1. Silently read the `_squad-ai/core/CATALOG.md` (or list skills from `_squad-ai/core/catalog.json`) to find installed skills that match the user's needs. Do NOT scan the `skills/` directory blindly.
2. Based on the squad's purpose and target formats, select which skills are relevant from the Catalog.
3. **Autonomous Skill Demand:** If the squad requires integrating with a specific platform, software, or methodology that does NOT currently exist in the Catalog, you MUST flag this demand. Include `requires_custom_skill: "<Name and brief purpose of the missing skill>"` in your internal context.
4. Save the auto-selected existing tools (and the newly proposed custom skills) in `tools_needed` — they will appear in the Step 7 summary where the user can adjust them.
...
Scan the `_squad-ai/core/best-practices/` directory at runtime. List ONLY the filenames — do NOT read or load the file contents. Ask: "Which formats interest you? Can be more than one."
...
## Output: `squads/{squad_code}/_build/discovery.yaml`
